package DynamicGraph.store.versionStore
...
import DynamicGraph.store.record.{DynamicVersionRecord, DynamicVersionRecordAllocator, DynamicVersionRecordFormat}
...
class DynamicVersionArrayStore(file: File, idFile: File,
  ...
  def allocateRecords(target: util.Collection[DynamicVersionRecord], array: Any): Unit = {
