...
import org.neo4j.internal.kernel.api.{CursorFactory, ExplicitIndexWrite, IndexReference, NodeCursor, SchemaWrite, Write}
...
class DynamicOperations(opr: Operations) extends Write with ExplicitIndexWrite with SchemaWrite{
  ...
  def nodeGetVersion(nodeId: Long): Long ={
    ...
    val nodes: NodeCursor = ktx.ambientNodeCursor()
    opr.nodeGetVersion(nodeId)
  }
