...
public class NodeVersionRecordFormat extends BaseOneByteHeaderRecordFormat<NodeVersionRecord> {
    public static final int RECORD_SIZE = 19;

    public NodeVersionRecordFormat() {
        super(fixedRecordSize(19), 0, 1, 35);
    }

    public NodeVersionRecord newRecord() {
        return new NodeVersionRecord(-1L);
    }

    public void read(NodeVersionRecord record, PageCursor cursor, RecordLoad mode, int recordSize) {
    ...
    public void write(NodeVersionRecord record, PageCursor cursor, int recordSize) {
        if (record.inUse()) {
