...
public class NodeStore extends CommonAbstractStore<NodeRecord, NoStoreHeader> {
    ...
    //DynamicGraph
    //**************************************************
    private final DynamicVersionArrayStore dynamicVersionLabelStore;
    ...
    public NodeStore(File file, File idFile, Config config, IdGeneratorFactory idGeneratorFactory, PageCache pageCache, LogProvider logProvider, DynamicArrayStore dynamicLabelStore,DynamicVersionArrayStore dynamicVersionLabelStore, RecordFormats recordFormats, OpenOption... openOptions) {
    ...
    public void ensureHeavyEx(NodeRecord node) {
        if (NodeVersionLabelsField.fieldPointsToDynamicRecordOfLabels(node.getLabelField())) {
            this.ensureHeavyEx(node, NodeVersionLabelsField.firstDynamicLabelRecordId(node.getLabelField()));
        }
    ...
    public DynamicVersionArrayStore getDynamicVersionLabelStore() {
        return this.dynamicVersionLabelStore;
    }


    public void updateDynamicVersionLabelRecords(Iterable<DynamicVersionRecord> dynamicLabelRecords) {
        Iterator var2 = dynamicLabelRecords.iterator();

        while(var2.hasNext()) {
            DynamicVersionRecord record = (DynamicVersionRecord)var2.next();
            this.dynamicVersionLabelStore.updateRecord(record);
        }
