...
public class GraphDatabaseFacade implements GraphDatabaseAPI, EmbeddedProxySPI {
    ...
    public void init(GraphDatabaseFacade.SPI spi, ThreadToStatementContextBridge txBridge, Config config, TokenHolders tokenHolders) {
        this.spi = spi;
        this.config = config;
   ...
   /* @Override
        ...
        if (query.propertyKeyId() != -1 && labelId != -1) {
            ...
            if (index != IndexReference.NO_INDEX) {
                try {
                    ...
                    return new GraphDatabaseFacade.NodeCursorResourceIterator(cursor, statement, this::newNodeProxy);
    ...
    private ResourceIterator<Node> getNodesByLabelAndPropertyWithoutIndex(Statement statement, int labelId, IndexQuery... queries) {
        ...
        return new GraphDatabaseFacade.NodeLabelPropertyIterator(transaction.dataRead(), nodeLabelCursor, nodeCursor, propertyCursor, statement, this::newNodeProxy, queries);
    }
    ...
    private abstract static class PrefetchingNodeResourceIterator implements ResourceIterator<Node> {
        ...
        private final GraphDatabaseFacade.NodeFactory nodeFactory;
        private long next;
        private boolean closed;
        private static final long NOT_INITIALIZED = -2L;
        protected static final long NO_ID = -1L;

        PrefetchingNodeResourceIterator(Statement statement, GraphDatabaseFacade.NodeFactory nodeFactory) {
    ...
    private static final class NodeCursorResourceIterator<CURSOR extends NodeIndexCursor> extends GraphDatabaseFacade.PrefetchingNodeResourceIterator {
        private final CURSOR cursor;

        NodeCursorResourceIterator(CURSOR cursor, Statement statement, GraphDatabaseFacade.NodeFactory nodeFactory) {
    ...
    private static class NodeLabelPropertyIterator extends GraphDatabaseFacade.PrefetchingNodeResourceIterator {
        ...
        NodeLabelPropertyIterator(Read read, NodeLabelIndexCursor nodeLabelCursor, NodeCursor nodeCursor, PropertyCursor propertyCursor, Statement statement, GraphDatabaseFacade.NodeFactory nodeFactory, IndexQuery... queries) {
            super(statement, nodeFactory);
            this.read = read;
