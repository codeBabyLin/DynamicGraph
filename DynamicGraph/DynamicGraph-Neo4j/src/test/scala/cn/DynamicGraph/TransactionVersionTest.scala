...
class TransactionVersionTest extends BaseTest {
  ...
  //todo getAllproperties property not delete really
...
/*
 ...
 /*   val nodes = graphDb.getAllNodes
  ...
  @Test
  def testNodeAddDeleteProperty(): Unit ={
    val path = "F:\\DynamicGraphStore"
    val dataBaseDir = new File(path,"data")
    graphDb = new GraphDatabaseFactory().newEmbeddedDatabase(dataBaseDir)
    registerShutdownHook(graphDb)
    ...
    val r1Id = r1.getId
    r1.setProperty("time",1998)
    r1.setProperty("size",500)
    ...
    //node1 = graphDb.findNode(label,"name","codeBaby")
    node1 = graphDb.getNodeById(nodeId)
    node1.getAllProperties.forEach(new BiConsumer[String,Object] {
      override def accept(t: String, u: Object): Unit = {
        println(s"tx:${DGVersion.getStartVersion(tx.getVersion)},node:${node1.getId},${t}:${u}")
      }
    })

    r1 = graphDb.getRelationshipById(r1Id)

    r1.getAllProperties.forEach(new BiConsumer[String,Object] {
      override def accept(t: String, u: Object): Unit = {
        println(s"tx:${DGVersion.getStartVersion(tx.getVersion)},rel:${r1.getId},${t}:${u}")
      }
    })



    r1.setProperty("size",300)
    node1.removeProperty("age")
/*    val obj = node1.getProperty("name")
    println(obj)*/
   // node1.setProperty("age",2077)
    tx.success()
    tx.close()
   ...
   // val objx = node1.getProperty("age",0)
   // println(objx)
  //  node1.setProperty("age",2077)
    tx.success()
    tx.close()
  ...
  def testioo(): Unit ={
   ...
   // println(obj)
    ...
    tx = graphDb.beginTx()//2
    node1 = graphDb.findNode(label,"name","codeBaby")
    var objx = node1.getProperty("age")
   // println(objx)
    ...
    tx = graphDb.beginTx()//3
    node1 = graphDb.findNode(label,"name","codeBaby")
    objx = node1.getProperty("age")
    //println(objx)

    node1.setProperty("age",2090)
    node1.setProperty("size",188)
    tx.success()
    tx.close()

    tx = graphDb.beginTx()//4
    node1 = graphDb.findNode(label,"name","codeBaby")
    objx = node1.getProperty("age")
    //println(objx)

    node1.setProperty("age",2091)

    tx.success()
    tx.close()


    tx = graphDb.beginTx()//5
    node1 = graphDb.findNode(label,"name","codeBaby")
    objx = node1.getProperty("age")
    //println(objx)

    node1.setProperty("age",2092)

    tx.success()
    tx.close()


    tx = graphDb.beginTx()//6
    node1 = graphDb.findNode(label,"name","codeBaby")
    objx = node1.getProperty("age")
    //println(objx)
    node1.setProperty("size",194)
    node1.setProperty("age",2093)

    tx.success()
    tx.close()


    tx = graphDb.beginTx()//7
    node1 = graphDb.findNode(label,"name","codeBaby")
    objx = node1.getProperty("age")
    //println(objx)

    node1.setProperty("age",2094)
    ...
    tx = graphDb.beginTx()//8
    node1 = graphDb.findNode(label,"name","codeBaby")
    println(DGVersion.getStartVersion(tx.getVersion))
    objx = node1.getProperty("age",6)
    val objxc = node1.getProperty("size",6)
