...
//    node.setVersionLabel(label1,1001)
...
//
//   // node.addLabel(label2)
//    node.setVersionLabel(label2,10086)
//    tx.success()
//    tx.close()
//
//    tx =  graphDb.beginTx()
...
//   // node.getVersionLabels(10086).foreach(println)
//
//    graphDb.shutdown()
//  }
//
//  @Test
//  def testNodeVersionALL(): Unit ={
//    val path = "F:\\DynamicGraphStore"
//    val dataBaseDir = new File(path,"data")
//    graphDb = new GraphDatabaseFactory().newEmbeddedDatabase(dataBaseDir)
//    registerShutdownHook(graphDb)
...
//    tx =  graphDb.beginTx()
//    node = graphDb.getNodeById(nodeId).asInstanceOf[DynamicNodeProxyM2]
//    //Assert.assertEquals(10086,node.getNodeVersion())
//    println(node.getNodeVersion())
//    Assert.assertEquals(18,node.getVersionProterty("age",1001))
//    Assert.assertEquals(2022,node.getVersionProterty("year",10086))
//    //Assert.assertEquals("boy",node.getVersionLabels(10086).head.name())
//    graphDb.shutdown()
//  }
//
//
